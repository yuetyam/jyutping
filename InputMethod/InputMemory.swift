...
struct InputMemory {
        ...
        private static func migrate(lower: Int, upper: Int) {
                ...
                let command: String = "INSERT OR IGNORE INTO core_memory (word, romanization, frequency, latest, shortcut, spell) VALUES (?, ?, ?, ?, ?, ?);"
        ...
        // memory(identifier INTEGER PRIMARY KEY, word TEXT, romanization TEXT, frequency INTEGER, latest INTEGER, anchors INTEGER, shortcut INTEGER, ping INTEGER)
        ...
        private static func ensureTable() {
                let command: String = "CREATE TABLE IF NOT EXISTS core_memory (id INTEGER PRIMARY KEY AUTOINCREMENT, word TEXT NOT NULL, romanization TEXT NOT NULL, frequency INTEGER NOT NULL, latest INTEGER NOT NULL, shortcut INTEGER NOT NULL, spell INTEGER NOT NULL, UNIQUE (word, romanization));"
                var statement: OpaquePointer? = nil
                defer { sqlite3_finalize(statement) }
                guard sqlite3_prepare_v2(database, command, -1, &statement, nil) == SQLITE_OK else { return }
        ...
        private static func insert(entry: MemoryLexicon) {
                let command: String = "INSERT INTO core_memory (word, romanization, frequency, latest, shortcut, spell) VALUES (?, ?, ?, ?, ?, ?);"
                var statement: OpaquePointer? = nil
                defer { sqlite3_finalize(statement) }
                guard sqlite3_prepare_v2(database, command, -1, &statement, nil) == SQLITE_OK else { return }
                sqlite3_bind_text(statement, 1, (entry.word as NSString).utf8String, -1, transientDestructorType)
