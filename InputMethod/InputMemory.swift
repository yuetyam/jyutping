...
struct InputMemory {
        ...
        private static func insert(entry: MemoryLexicon) {
                let leading: String = "INSERT INTO memory (identifier, word, romanization, frequency, latest, anchors, shortcut, ping) VALUES ("
                let trailing: String = ");"
                let values: String = "\(entry.identifier), '\(entry.word)', '\(entry.romanization)', \(entry.frequency), \(entry.latest), \(entry.anchors), \(entry.shortcut), \(entry.ping)"
