...
struct CharacterVariant {
        struct VariantMap: Hashable {
                let left: UInt32
                let right: UInt32
                static func ==(lhs: VariantMap, rhs: VariantMap) -> Bool {
        ...
        static func generate(_ sourceUrl: URL) -> [VariantMap] {
                ...
                let sourceLines: [String] = sourceContent
                        ...
                        .distinct()
                let variantMaps = sourceLines.compactMap({ line -> VariantMap? in
                        let errorMessage: String = "Bad format in line text: \(line)"
                        let warningMessage: String = "Not Generic Ideographic: \(line)"
                        ...
                        case 1:
                                ...
                                guard leftCode.isGenericCJKVCodePoint && rightCode.isGenericCJKVCodePoint else {
                        ...
                        default:
                                ...
                                guard let leftCode = leftChar.unicodeScalars.first?.value else { fatalError(errorMessage) }
                                guard leftCode.isGenericCJKVCodePoint else {
                                        print(warningMessage)
                                        return nil
                                }
                                ...
                                guard let rightChar = rightComponents.first?.first else { fatalError(errorMessage) }
                                guard let rightCode = rightChar.unicodeScalars.first?.value else { fatalError(errorMessage) }
                                guard rightCode.isGenericCJKVCodePoint else {
                                        print(warningMessage)
                                        return nil
                                }
                                guard rightCode != leftCode else { return nil }
                                return VariantMap(left: leftCode, right: rightCode)
                        }
                })
        ...
        @available(*, unavailable, message: "Use generate() instead")
        private static func process(_ sourceUrl: URL) -> [VariantMap] {
                guard let sourceContent: String = try? String(contentsOf: sourceUrl, encoding: .utf8) else { fatalError("Can not read file from URL: \(sourceUrl)") }
                let sourceLines: [String] = sourceContent
                        .trimmingCharacters(in: .whitespacesAndNewlines)
                        .components(separatedBy: .newlines)
